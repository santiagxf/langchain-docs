...
This page covers Microsoft Foundry project tools from `langchain_azure_ai.tools`. See also the tools provided as part of [Microsoft Foundry Tools (formerly Azure AI Services)](azure_ai_services).
...
| [AzureAIProjectToolbox](#azure-ai-foundry-toolbox) | Load tools from an Azure AI Foundry Toolbox and use them via Model Context Protocol (MCP). |
...
`ImageGenerationTool` allows the model to generate or edit images using GPT image models. This is useful for creating visuals, editing images, and generating artwork based on text descriptions. This tool must be used with an OpenAI model deployed in a Microsoft Foundry project. If you are using other model, use [AzureOpenAIModelImageGenTool](#AzureOpenAIModelImageGenTool) instead.
