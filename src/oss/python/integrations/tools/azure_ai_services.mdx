...
Microsoft Foundry Tools (formerly known as Azure AI Services) wrap Azure AI service APIs for agent tool use. These tools live in the `langchain-azure-ai` package, are exported from `langchain_azure_ai.tools`, and can be instantiated individually or loaded together with `AIServicesToolkit`.
...
- `AIServicesToolkit` for loading all service tools at once.
...
This extra installs the service-specific dependencies used by these tools, including `azure-ai-contentunderstanding`, `azure-ai-documentintelligence`, `azure-ai-vision-imageanalysis`, `azure-cognitiveservices-speech`, and `azure-ai-textanalytics`.
